    ...
    for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex++)
    {
        ...
        if (bLimitKpts)
        {
            ...
            switch(detectorType){
                ...
                case DetectorType::HARRIS: ;
                case DetectorType::ORB: ;
                case DetectorType::SHITOMASI: keypoints.erase(keypoints.begin() + maxKeypoints, keypoints.end());       // there is no response info, so keep the first 50 as they are sorted in descending quality order
                case DetectorType::SIFT: ;
            }
        ...
        if (dataBuffer.size() > 1) // wait until at least two images have been processed
        {
            ...
            if (bVis)
            {
                ...
                cv::drawMatches(previousFrame.cameraImg, previousFrame.keypoints,
                                currentFrame.cameraImg, currentFrame.keypoints,
                                matches, matchImg,
